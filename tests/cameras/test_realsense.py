...
def test_read_rgbd_pair():
    config = RealSenseCameraConfig(
        serial_number_or_name="042", width=640, height=480, fps=30, use_depth=True, warmup_s=0
    )
    with RealSenseCamera(config) as camera:
        rgb, depth, timestamp = camera.read_rgbd(timeout_ms=2000)
...
def test_read_latest_rgbd_pair():
    config = RealSenseCameraConfig(
        serial_number_or_name="042", width=640, height=480, fps=30, use_depth=True, warmup_s=0
    )
    with RealSenseCamera(config) as camera:
        _ = camera.read_rgbd(timeout_ms=2000)
        rgb, depth, timestamp = camera.read_latest_rgbd(max_age_ms=1000)
        assert isinstance(rgb, np.ndarray)
        assert isinstance(depth, np.ndarray)
        assert rgb.shape[:2] == depth.shape[:2]
        assert isinstance(timestamp, float)
