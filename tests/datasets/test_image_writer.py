...
def test_image_array_to_pil_image_single_channel(img_array_factory):
    ...
    result_image = image_array_to_pil_image(img_array)
    assert isinstance(result_image, Image.Image)
    assert result_image.mode in {"L", "I;16"}
    assert np.array(result_image).ndim == 2


def test_image_array_to_pil_image_depth_uint16():
    img_array = (np.arange(100, dtype=np.uint16).reshape(10, 10) * 10)
    result_image = image_array_to_pil_image(img_array)
    assert isinstance(result_image, Image.Image)
    saved = np.array(result_image)
    assert saved.dtype == np.uint16
    assert np.array_equal(saved, img_array)
