...
def mock_load_image_as_numpy(path, dtype, channel_first):
    target_dtype = np.uint8 if dtype is None else dtype
    return np.ones((3, 32, 32), dtype=target_dtype) if channel_first else np.ones((32, 32, 3), dtype=target_dtype)
...
def test_sample_images_depth_preserves_uint16(tmp_path):
    image_paths = []
    for i in range(4):
        depth = np.full((16, 16), i * 1000, dtype=np.uint16)
        path = tmp_path / f"depth_{i}.png"
        from PIL import Image

        Image.fromarray(depth).save(path)
        image_paths.append(str(path))

    images = sample_images(image_paths, dtype=None)
    assert images.shape[1:] == (1, 16, 16)
...
def test_compute_episode_stats_with_depth_image_data(tmp_path):
    image_paths = []
    for i in range(5):
        depth = np.full((8, 8), 1000 + i, dtype=np.uint16)
        path = tmp_path / f"depth_{i}.png"
        from PIL import Image
    ...
    episode_data = {
        "observation.images.front_depth": image_paths,
        "action": np.random.normal(0, 1, (5, 2)),
    }
    features = {
        "observation.images.front_depth": {"dtype": "video", "shape": (8, 8, 1)},
        "action": {"dtype": "float32", "shape": (2,)},
    }

    stats = compute_episode_stats(episode_data, features)

    assert stats["observation.images.front_depth"]["mean"].shape == (1, 1, 1)
    assert stats["observation.images.front_depth"]["mean"].dtype == np.float64
    assert stats["observation.images.front_depth"]["mean"][0, 0, 0] > 1000
