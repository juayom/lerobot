...
def _decode_video_frames_pyav_compat(
    video_path: Path | str,
    timestamps: list[float],
    tolerance_s: float,
    log_loaded_timestamps: bool = False,
) -> torch.Tensor:
    """PyAV-based compatibility fallback when torchvision VideoReader is unavailable.

    Some torchvision builds (notably certain arm64 / packaged builds) omit `torchvision.io.VideoReader`.
    In that case, decode the requested time range directly with PyAV.
    """
    video_path = str(video_path)
    ...
    with av.open(video_path, "r") as container:
        ...
        if stream.time_base is not None:
            seek_target = int(first_ts / float(stream.time_base))
    ...
    if not is_within_tol.all():
        logging.warning(
            "pyav-compat timestamp mismatch exceeds tolerance; using nearest decoded frame anyway. "
            "dist=%s tolerance=%s video=%s backend=pyav-compat",
            min_[~is_within_tol],
            tolerance_s,
            video_path,
        )
...
def decode_video_frames_torchvision(
    ...
    if torchvision is None or not hasattr(torchvision.io, "VideoReader"):
        ...
        return _decode_video_frames_pyav_compat(
            video_path, timestamps, tolerance_s, log_loaded_timestamps=log_loaded_timestamps
        )
