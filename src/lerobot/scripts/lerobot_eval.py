...
from lerobot.policies.utils import infer_visual_rename_map
...
def eval_main(cfg: EvalPipelineConfig):
    ...
    env_policy_features = env_to_policy_features(cfg.env)
    effective_rename_map = infer_visual_rename_map(cfg.policy, env_policy_features, cfg.rename_map)
    inferred_only_map = {
        key: value for key, value in effective_rename_map.items() if cfg.rename_map.get(key) != value
    }
    if inferred_only_map:
        logging.info("Auto-inferred visual rename map: %s", inferred_only_map)
