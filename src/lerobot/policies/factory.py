...
def make_policy(
    ...
    inferred_only_map = {
        key: value
        for key, value in effective_rename_map.items()
        if (rename_map or {}).get(key) != value
    }
    if inferred_only_map:
        logging.info(
            "Auto-inferred visual rename map for %s: %s",
            cfg.type,
            inferred_only_map,
        )

    validate_visual_features_consistency(cfg, features, rename_map=effective_rename_map)
