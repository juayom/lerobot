...
def _extract_visual_feature_keys(features: dict[str, PolicyFeature] | None) -> list[str]:
    if not features:
        return []
    return [key for key, ft in features.items() if ft.type == FeatureType.VISUAL]


def rename_policy_features(
    features: dict[str, PolicyFeature],
    rename_map: dict[str, str] | None = None,
) -> dict[str, PolicyFeature]:
    """Return a copy of feature definitions with top-level keys renamed."""
    if not rename_map:
        return dict(features)
    return {rename_map.get(key, key): ft for key, ft in features.items()}



def infer_visual_rename_map(
    cfg: PreTrainedConfig,
    features: dict[str, PolicyFeature],
    rename_map: dict[str, str] | None = None,
) -> dict[str, str]:
    """Infer a stable camera-key rename map for any visual policy.

    Some checkpoints expect canonical visual keys such as
    ``observation.images.image1`` or ``observation.images.base_0_rgb`` even when a
    dataset or runtime observation uses different camera names. This helper preserves
    any user-provided renames and then fills remaining unmatched visual keys by order,
    so policies can consume arbitrary but consistent ``observation.images.*`` keys
    without an explicit ``--rename_map``.
    """
    effective_map = dict(rename_map or {})

    expected_visuals = _extract_visual_feature_keys(cfg.input_features)
    provided_visuals = _extract_visual_feature_keys(features)
    if not expected_visuals or not provided_visuals:
        return effective_map

    remapped_provided = [effective_map.get(key, key) for key in provided_visuals]
    satisfied_expected = {key for key in remapped_provided if key in expected_visuals}

    available_expected = [key for key in expected_visuals if key not in satisfied_expected]
    available_sources = [
        key for key in provided_visuals if key not in effective_map and key not in expected_visuals
    ]

    for source_key, target_key in zip(available_sources, available_expected, strict=False):
        effective_map[source_key] = target_key
...
def validate_visual_features_consistency(
    ...
    For visual policies, a best-effort visual-key alias map is inferred before
    validation so arbitrary dataset camera names can line up with the checkpoint's
    canonical camera slots.

    Args:
        cfg (PreTrainedConfig): The model or policy configuration containing input_features and type.
        features (Dict[str, PolicyFeature]): A mapping of feature names to PolicyFeature objects.
        rename_map: Optional explicit rename map supplied by the user.
    """
    effective_rename_map = infer_visual_rename_map(cfg, features, rename_map)
    remapped_features = rename_policy_features(features, effective_rename_map)

    expected_visuals = set(_extract_visual_feature_keys(cfg.input_features))
    provided_visuals = set(_extract_visual_feature_keys(remapped_features))
