...
class RealSenseCamera(Camera):
    ...
    def __init__(self, config: RealSenseCameraConfig):
        ...
        self.latest_pair_sequence: int = 0
        self._last_consumed_pair_sequence: int = -1
            ...
            try:
                ...
                with self.frame_lock:
                    ...
                    if self.use_depth:
                        ...
                        self.latest_depth_timestamp = capture_time
                    else:
                        self.latest_depth_frame = None
                        self.latest_depth_timestamp = None
                    self.latest_pair_sequence += 1
    ...
    def _stop_read_thread(self) -> None:
        ...
        with self.frame_lock:
            ...
            self.latest_depth_timestamp = None
            self.latest_pair_sequence = 0
            self._last_consumed_pair_sequence = -1
            self.new_frame_event.clear()
    ...
    def async_read(self, timeout_ms: float = 200) -> NDArray[Any]:
        ...
        with self.frame_lock:
            ...
            self._last_consumed_pair_sequence = self.latest_pair_sequence
            self.new_frame_event.clear()
    ...
    def read_latest(self, max_age_ms: int = 1000) -> NDArray[Any]:
        ...
        """
        frame, _timestamp = self._get_latest_color_frame(max_age_ms=max_age_ms)
        return frame

    def _get_latest_color_frame(self, max_age_ms: int = 1000) -> tuple[NDArray[Any], float]:
        if not self.is_connected:
            raise DeviceNotConnectedError(f"{self} is not connected.")
    ...
    def read_rgbd(self, timeout_ms: float = 200) -> tuple[NDArray[Any], NDArray[Any], float]:
        """Return a coherent RGB-D pair from the latest captured RealSense frames.

        This waits for a new frame bundle from the background thread, then returns
        the color image, the depth map, and the capture timestamp associated with
        that bundle.
        """
        if not self.use_depth:
        ...
        if color is None or depth is None or timestamp is None:
            raise RuntimeError(f"{self} has not captured a complete RGB-D pair yet.")

        return color.copy(), depth.copy(), timestamp

    def read_latest_rgbd(self, max_age_ms: int = 1000) -> tuple[NDArray[Any], NDArray[Any], float]:
        """Return the most recent RGB-D pair immediately without waiting for a new frame."""
        if not self.use_depth:
            raise RuntimeError(f"Depth stream is not enabled for {self}.")

        color, timestamp = self._get_latest_color_frame(max_age_ms=max_age_ms)
        with self.frame_lock:
            depth = self.latest_depth_frame
            depth_timestamp = self.latest_depth_timestamp

        if depth is None or depth_timestamp is None:
            raise RuntimeError(f"{self} has not captured any depth frames yet.")

        depth_age_ms = (time.perf_counter() - depth_timestamp) * 1e3
        if depth_age_ms > max_age_ms:
            raise TimeoutError(
                f"{self} latest depth frame is too old: {depth_age_ms:.1f} ms (max allowed: {max_age_ms} ms)."
            )
